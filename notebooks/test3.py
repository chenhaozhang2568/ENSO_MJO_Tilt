...
MOD (minimal, per request):
1) Reconstruct ERA5 w (low/up, latmean) using the SAME Step3 PC1/PC2 via linear regression:
     w_mjo(t,lon) = a(lon) + b1(lon)*pc1(t) + b2(lon)*pc2(t)
2) Normalize by daily MJO amplitude:
     w_mjo_norm = w_mjo / amp
3) Use w_mjo_norm (instead of raw w_bp) to detect ascent edges/centers and compute tilt.

Inputs:
- Step3 NC: center_lon_track(time), pc1(time), pc2(time), amp(time), olr_center_track(time)
...
def main():
    ...
    # subset lon to tracking window and select two layers
