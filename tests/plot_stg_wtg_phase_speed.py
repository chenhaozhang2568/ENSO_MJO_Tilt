# -*- coding: utf-8 -*-
"""
plot_stg_wtg_phase_speed.py
按 tilt_q 强弱分组（STG/WTG），比较两组相速度差异

分组标准：
  STG (Strong Tilting): tilt_q > mean + 0.7*std
  WTG (Weak Tilting):   tilt_q < mean - 0.7*std
  中间组不参与比较
"""
import pandas as pd
import numpy as np
...
CSV = r"E:\Datas\Derived\tilt_q_phase_speed_by_enso.csv"
OUT_DIR = r"E:\Projects\ENSO_MJO_Tilt\outputs\figures\enso_tilt_q"
SIGMA_THRESH = 0.7


def main():
    df = pd.read_csv(CSV)
    tilt = df["mean_tilt_q"].values
    ...
    stg = df[df["mean_tilt_q"] >= hi]
    wtg = df[df["mean_tilt_q"] <= lo]
    mid = df[(df["mean_tilt_q"] > lo) & (df["mean_tilt_q"] < hi)]

    print(f"Tilt_q: mean={mean_t:.2f}, std={std_t:.2f}")
    ...
    # --- Left: scatter tilt_q vs speed, colored by group ---
    ax = axes[0]
    ax.scatter(mid["mean_tilt_q"], mid["phase_speed_m_s"],
               c="#CCCCCC", s=25, alpha=0.5, label="Middle", edgecolors="none")
    ax.scatter(wtg["mean_tilt_q"], wtg["phase_speed_m_s"],
               c="#3498DB", s=40, alpha=0.8, label=f"WTG (n={len(wtg)})",
               edgecolors="black", linewidths=0.3)
    ax.scatter(stg["mean_tilt_q"], stg["phase_speed_m_s"],
    ...
    ax.set_xlabel("Tilt_q (deg)", fontsize=12)
    ax.set_ylabel("Phase Speed (m/s)", fontsize=12)
    ax.set_title("Tilt_q vs Phase Speed (STG/WTG)", fontsize=13, fontweight="bold")
