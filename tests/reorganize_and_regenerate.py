# -*- coding: utf-8 -*-
"""
reorganize_and_regenerate.py
整理 tilt_q_diagnose 输出：
  1. 将现有平均场版结果移入 both_meanfield/ 子文件夹
  2. 重新生成逐日上层均值版结果到 upper_daily_lower_meanfield/ 子文件夹

两个文件夹各包含：event_profile/ + centroid_profile/ + 6张两两对比散点图
"""
import shutil
import numpy as np
import pandas as pd
...
BOTH_MF_DIR = BASE_DIR / "both_meanfield"
DAILY_UP_DIR = BASE_DIR / "upper_daily_lower_meanfield"
...
# ==============================
# 辅助函数（复用自 03b_diagnose_tilt_q.py）
# ==============================
...
def _compute_daily_up_west(data, event_indices, centers):
    ...
    for idx, c in zip(event_indices, centers):
        ...
        up_west_rels.append(_ascent_boundary_zero(target_rel, w_up_smooth))
    return np.array(up_west_rels)
...
def plot_profile(eid, w_mean, q_mean, target_rel, target_h,
    ...
    ax.set_title(f"Event #{eid_v}: {event_row['start_date']} ~ {event_row['end_date']} "
                 f"({int(event_row['duration_days'])}d)", fontsize=13, fontweight='bold')
...
def main():
    print("=" * 60)
    print("Step 1: 移动现有平均场版结果到 both_meanfield/")
    print("=" * 60)

    # 创建目标文件夹
    bf_ep = BOTH_MF_DIR / "event_profile"
    bf_cp = BOTH_MF_DIR / "centroid_profile"
    bf_ep.mkdir(parents=True, exist_ok=True)
    bf_cp.mkdir(parents=True, exist_ok=True)

    # 移动 event_profile/
    src_ep = BASE_DIR / "event_profile"
    if src_ep.exists():
        for f in src_ep.glob("*.png"):
            shutil.move(str(f), str(bf_ep / f.name))
        print(f"  Moved event_profile/ → both_meanfield/event_profile/ ({len(list(bf_ep.glob('*.png')))} files)")

    # 移动 centroid_profile/
    src_cp = BASE_DIR / "centroid_profile"
    if src_cp.exists():
        for f in src_cp.glob("*.png"):
            shutil.move(str(f), str(bf_cp / f.name))
        print(f"  Moved centroid_profile/ → both_meanfield/centroid_profile/ ({len(list(bf_cp.glob('*.png')))} files)")

    # 移动 6 张散点图
    scatter_files = [
        "up_west_vs_phase_speed.png", "q_max_vs_phase_speed.png", "up_west_vs_q_max.png",
        "centroid_vs_speed.png", "omega_west_vs_speed.png", "omega_west_vs_centroid.png",
    ]
    for fname in scatter_files:
        src = BASE_DIR / fname
        if src.exists():
            shutil.move(str(src), str(BOTH_MF_DIR / fname))
    print(f"  Moved 6 scatter plots → both_meanfield/")

    # 移动 CSV
    csv_src = BASE_DIR / "event_mean_field_values.csv"
    if csv_src.exists():
        shutil.move(str(csv_src), str(BOTH_MF_DIR / "event_mean_field_values.csv"))

    print("\n" + "=" * 60)
    print("Step 2: 重新生成逐日上层均值版 → upper_daily_lower_meanfield/")
    print("=" * 60)

    du_ep = DAILY_UP_DIR / "event_profile"
    du_cp = DAILY_UP_DIR / "centroid_profile"
    du_ep.mkdir(parents=True, exist_ok=True)
    du_cp.mkdir(parents=True, exist_ok=True)
    ...
    for ev_idx, row in events.iterrows():
        ...
        if len(event_indices) < 2:
            all_daily_uw.append(np.nan)
            all_centroids.append(np.nan)
            all_qmax_field.append(np.nan)
            continue
        ...
        mean_daily_uw = float(np.nanmean(daily_uw))
        all_daily_uw.append(mean_daily_uw)

        # 平均场下层
        low_h_max = LEVEL_TO_HEIGHT[850]
        h_mask = target_h <= low_h_max
        q_low_profile = np.nanmean(q_mean[h_mask, :], axis=0)
        centroid = _find_q_centroid(target_rel, q_low_profile)
        all_centroids.append(centroid)
        qmax_field = _find_q_max(target_rel, q_low_profile,
                                  Q_MAX_SEARCH_RANGE[0], Q_MAX_SEARCH_RANGE[1])
        all_qmax_field.append(qmax_field)

        # 逐日 q_max (for scatter on event_profile)
        q_max_rels = []
        for idx, c in zip(event_indices, centers):
            if not np.isfinite(c):
                q_max_rels.append(np.nan); continue
            rel_q = data['lon_q_360'] - c
            mq = (rel_q >= REL_LON_RANGE[0]) & (rel_q <= REL_LON_RANGE[1])
            q_day = data['q_raw'][idx, :, :][:, data['q_sort']][:, mq]
            low_mask = (data['levels_q'] >= Q_LOW_LAYER[1]) & (data['levels_q'] <= Q_LOW_LAYER[0])
            q_low = np.nanmean(q_day[low_mask, :], axis=0)
            q_interp = _cubic_spline_interp_1d(rel_q[mq], q_low, target_rel)
            q_smooth = _smooth_1d(q_interp, SMOOTH_WINDOW)
            q_max_rels.append(_find_q_max(target_rel, q_smooth, Q_MAX_SEARCH_RANGE[0], Q_MAX_SEARCH_RANGE[1]))
        q_max_rels = np.array(q_max_rels)

        # event_profile: daily_uw + q_max_field
        plot_profile(eid, w_mean, q_mean, target_rel, target_h,
                     mean_daily_uw, qmax_field, 'q max', 'darkgreen',
                     row, du_ep / f"event_{eid:03d}_profile.png",
                     scatter_uw=daily_uw, scatter_qm=q_max_rels)

        # centroid_profile: daily_uw + centroid
        plot_profile(eid, w_mean, q_mean, target_rel, target_h,
                     mean_daily_uw, centroid, 'q centroid', '#2471A3',
                     row, du_cp / f"event_{eid:03d}_centroid_profile.png")

        print(f"  Event #{eid} (daily_uw={mean_daily_uw:.1f}°, centroid={centroid:.1f}°)")

    # 6 张散点图
    print("\n  Generating 6 scatter plots...")
    all_daily_uw = np.array(all_daily_uw)
    all_centroids = np.array(all_centroids)
    all_qmax_field = np.array(all_qmax_field)

    df_vals = pd.DataFrame({
        'event_id': events['event_id'].values,
        'daily_up_west': all_daily_uw,
        'field_centroid': all_centroids,
        'field_q_max': all_qmax_field,
    })
    df = df_ps.merge(df_vals, on="event_id")
    speed = df["phase_speed_m_s"].values
    uw = df["daily_up_west"].values
    qm = df["field_q_max"].values
    ct = df["field_centroid"].values

    plot_scatter(speed, uw, "Speed (m/s)", "Upper ω West Boundary (°)",
                 "phase speed", "upper ω west (daily mean)",
                 DAILY_UP_DIR / "up_west_vs_phase_speed.png")
    plot_scatter(speed, qm, "Speed (m/s)", "Lower q Max Position (°)",
                 "phase speed", "lower q max (mean-field)",
                 DAILY_UP_DIR / "q_max_vs_phase_speed.png")
    plot_scatter(uw, qm, "Upper ω West Boundary (°)", "Lower q Max Position (°)",
                 "upper ω west (daily mean)", "lower q max (mean-field)",
                 DAILY_UP_DIR / "up_west_vs_q_max.png")
    plot_scatter(speed, ct, "Speed (m/s)", "q Centroid Position (°)",
                 "phase speed", "q centroid (mean-field)",
                 DAILY_UP_DIR / "centroid_vs_speed.png")
    plot_scatter(speed, uw, "Speed (m/s)", "ω West Boundary (°)",
                 "phase speed", "ω west (daily mean)",
                 DAILY_UP_DIR / "omega_west_vs_speed.png")
    plot_scatter(uw, ct, "ω West Boundary (°)", "q Centroid Position (°)",
                 "upper ω west (daily mean)", "q centroid (mean-field)",
                 DAILY_UP_DIR / "omega_west_vs_centroid.png")

    # 保存 CSV
    df_vals.to_csv(DAILY_UP_DIR / "event_daily_upper_values.csv", index=False)

    print(f"\n{'='*60}")
    print(f"Done! Results in:")
    print(f"  {BOTH_MF_DIR}")
    print(f"  {DAILY_UP_DIR}")
