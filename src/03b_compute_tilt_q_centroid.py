# -*- coding: utf-8 -*-
"""
03b_compute_tilt_q_centroid.py: 逐日 MJO Tilt 指数计算（q 重心定义下层）
...
Tilt 定义（centroid 版）：
    Tilt_q_centroid = q 低层平均重心经度 - omega 高层上升区西边界经度（相对经度，单位：°）

    - 上层边界（不变）：omega 400-200 hPa 层平均上升区西边界
    - 下层边界（新）：q 1000-850 hPa 层平均的**加权重心**经度位置
      centroid = Σ(q * lon) / Σ(q)，仅对 q>0 的区域计算
...
输出：
    - tilt_q_centroid_daily_1979-2022.nc
"""
...
OUT_NC = r"E:\Datas\Derived\tilt_q_centroid_daily_1979-2022.nc"
...
# ====================== 核心差异：使用重心（centroid）替代最大值 ======================
Q_CENTROID_SEARCH_MIN = 0.0    # 重心搜索范围：相对经度下界（°）
Q_CENTROID_SEARCH_MAX = 50.0   # 重心搜索范围：相对经度上界（°）


def _find_q_centroid(rel_lon: np.ndarray, q_profile: np.ndarray) -> tuple:
    """
    在 q 低层平均剖面中计算 q>0 区域的加权重心位置。
    搜索范围限制在相对经度 [Q_CENTROID_SEARCH_MIN, Q_CENTROID_SEARCH_MAX] 内。

    centroid = Σ(q * lon) / Σ(q)，仅对 q > 0 的部分计算。

    Returns: (q_centroid_rel_lon, q_sum_value)
    """
    m = (np.isfinite(q_profile) & np.isfinite(rel_lon)
         & (rel_lon >= Q_CENTROID_SEARCH_MIN) & (rel_lon <= Q_CENTROID_SEARCH_MAX)
         & (q_profile > 0))
    if m.sum() < MIN_VALID_POINTS:
    ...
    q_sum = float(np.sum(qq))
    if q_sum < 1e-20:
        return (np.nan, np.nan)

    centroid = float(np.sum(qq * rr) / q_sum)
    return (centroid, q_sum)
...
def main():
    ...
    # pre-allocate
    n = time.size
    q_centroid_rel = np.full(n, np.nan, np.float32)
    q_centroid_sum = np.full(n, np.nan, np.float32)
    ...
    tilt_q_centroid = np.full(n, np.nan, np.float32)
    ...
    for i in range(n):
        ...
        # q 低层重心位置（核心差异）
        ql = q_low_norm[i, :]
        qc_rel, qc_sum = _find_q_centroid(rel, ql)
        q_centroid_rel[i] = qc_rel
        q_centroid_sum[i] = qc_sum

        # 新 tilt：q_centroid_rel - up_west
        if np.isfinite(qc_rel) and np.isfinite(uw):
            tilt_q_centroid[i] = float(qc_rel - uw)
        else:
            tilt_q_centroid[i] = np.nan

    # --- save ---
    ds_out = xr.Dataset(
        {
            "q_centroid_rel": xr.DataArray(q_centroid_rel, coords={"time": time}, dims=("time",),
                          attrs={"desc": "q 低层平均加权重心相对经度 (deg)"}),
            "q_centroid_sum": xr.DataArray(q_centroid_sum, coords={"time": time}, dims=("time",),
                            attrs={"desc": "q 低层平均 q>0 区域总和"}),
            ...
            "tilt_q_centroid": xr.DataArray(tilt_q_centroid, coords={"time": time}, dims=("time",),
                       attrs={"desc": "tilt_q_centroid = q_centroid_rel - up_west_rel (deg), 水汽重心定义下层"}),
        ...
        attrs={
            ...
            "boundary_method_low": "q centroid (weighted mean position of q>0)",
    ...
    tv = ds_out["tilt_q_centroid"].values.astype(float)
    ok = np.isfinite(tv)
    print("Saved:", str(out_path))
    print("tilt_q_centroid finite days:", int(ok.sum()), "/", int(tv.size),
          "winter:", int(winter.sum()))
    if ok.any():
        print("tilt_q_centroid stats (deg): min", float(np.nanmin(tv)), "p5", float(np.nanpercentile(tv, 5)),
              "mean", float(np.nanmean(tv)), "median", float(np.nanpercentile(tv, 50)),
              "p95", float(np.nanpercentile(tv, 95)), "max", float(np.nanmax(tv)))

    # --- q_centroid_rel summary ---
    qr = ds_out["q_centroid_rel"].values.astype(float)
    qok = np.isfinite(qr)
    if qok.any():
        print("q_centroid_rel stats (deg): min", float(np.nanmin(qr)), "p5", float(np.nanpercentile(qr, 5)),
