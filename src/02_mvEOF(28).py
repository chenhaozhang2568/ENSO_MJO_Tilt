...
MOD (per request, minimal):
- Keep ALL existing steps (MV-EOF, OLR reconstruction, DP center tracking) unchanged.
- ONLY change "eastward propagation / event selection" from:
    (center_track displacement)
  to:
    (intensity contour track based on reconstructed OLR threshold, i.e. -15 W m^-2 line)

Paper text: "the OLR contour with the aforementioned strength must move continually across 50° in longitude..."
Implementation here:
- Use reconstructed OLR lat-mean (time,lon)
- Define active region each day: recon_olr <= OLR_MIN_THRESH
- Choose the strongest contiguous active segment (by minimum OLR within each segment)
- Track its centroid longitude as contour_lon_track(t) (NaN if no segment)
- Build event segments from contour_lon_track continuity (allow TRACK_GAP_DAYS) and west-jump rule (MAX_WEST_JUMP_DEG)
- Keep other criteria unchanged: MIN_EAST_DISP_DEG, gates (IO/MC), AMP_THRESH(optional), MIN_ACTIVE_DAYS_IN_EVENT
...
U_BP_PATH   = r"E:\Datas\ERA5\processed\era5_u850_u200_bp_1979-2022.nc"
...
END_DATE   = "2022-12-31"  # later change to "2022-12-31"
...
# ==========================================================
# NEW: -15 contour based track (paper-style)
# ==========================================================
def _contour_track_from_threshold(
    recon_latmean: xr.DataArray,
    thr: float
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Given recon_latmean(time,lon), define active region by recon<=thr.
    For each day, pick the strongest contiguous active segment (by minimum OLR in segment).
    Return:
      west_lon[t], east_lon[t], centroid_lon[t]  (NaN if no active segment)
    """
    rr = recon_latmean.values.astype(float)  # (T,L)
    ...
    for t in range(T):
        ...
        # find contiguous segments of True along lon
        ...
        wlon = float(lon[int(best_seg[0])])
        elon = float(lon[int(best_seg[-1])])
        clon = float(np.nanmean(lon[best_seg]))
...
def _build_events_by_contour_track(
    time: pd.DatetimeIndex,
    contour_track: np.ndarray,
    active_flag: np.ndarray,
    amp: np.ndarray | None,
    winter_mask: np.ndarray
) -> pd.DataFrame:
    """
    Paper-style event selection using -15 contour "movement":
      - contour_track(t): representative longitude of the -15 contour (here: centroid of strongest active segment)
      - active_flag(t): whether there exists any lon with recon<=thr on that day

    Segment definition (within winter):
      - contiguous valid contour_track, allowing TRACK_GAP_DAYS NaN gap
      - west-jump constraint on successive valid points: jump >= -MAX_WEST_JUMP_DEG (else cut)
      - (optionally) AMP_THRESH

    Filters (unchanged):
      - disp = max(track) - min(track) >= MIN_EAST_DISP_DEG
      - gates: min(track) <= IO_GATE_LON and max(track) >= MC_GATE_LON
      - active days in segment >= MIN_ACTIVE_DAYS_IN_EVENT
    """
    n = len(time)
    if winter_mask.size != n:
        raise ValueError("winter_mask length mismatch")

    # active days only for counting and segment validation
    ...
    def finalize_segment(s0: int, s1: int):
        ...
        # lon_start/lon_end: first/last valid track point
    ...
    for t in range(n):
        ...
        # Only days with an active contour are meaningful; otherwise treat as gap
    ...
    print("  fail_short (no valid contour in seg):", int(fail_short))
...
def main():
    ...
    if SKIP_EOF_IF_EXISTS and EOF_CACHE_NC.exists():
        ...
        # 从缓存的 recon OLR 重新计算 contour track（用于这个版本的事件识别）
        print("[重建 contour track...]")
        west_lon, east_lon, contour_lon = _contour_track_from_threshold(
            recon_latmean=recon_olr_latmean,
            thr=OLR_MIN_THRESH
        )
        ...
        print("冬季活跃 contour 天数:", int(np.nansum(contour_active_w.astype(int))))
        
        # 事件识别
        df_events = _build_events_by_contour_track(
            time=time_index,
            contour_track=contour_lon_w,
            active_flag=contour_active_w,
            amp=amp_np,
    ...
    # ---- restrict lon to tracking window for OLR ----
    ...
    phase[~np.isfinite(pc1) | ~np.isfinite(pc2)] = 0

    # ---- reconstructed OLR (OLR block only) ----
    EOFs_olr = EOFs[:P, :]
    ...
    mu_olr_flat = mu_olr.values.astype(np.float64).reshape(P)
    recon_olr_flat = recon_std_flat * s_olr + mu_olr_flat[None, :]
    ...
    # ---- DP center track (kept, for Step4 alignment use) ----
    ...
    # ==========================================================
    # CHANGE HERE: event selection uses -15 contour movement
    # ==========================================================
    west_lon, east_lon, contour_lon = _contour_track_from_threshold(
        recon_latmean=recon_olr_latmean,
        thr=OLR_MIN_THRESH
    ...
    # active flag for contour existence
    contour_active = np.isfinite(contour_lon)

    # enforce non-winter = ignored
    contour_lon_w = contour_lon.copy()
    contour_west_w = west_lon.copy()
    contour_east_w = east_lon.copy()
    contour_active_w = contour_active.copy()

    contour_lon_w[~winter_np] = np.nan
    contour_west_w[~winter_np] = np.nan
    contour_east_w[~winter_np] = np.nan
    contour_active_w[~winter_np] = False
    ...
    print("Winter days with any lon<=thr (contour_active):", int(np.nansum(contour_active_w.astype(int))))
    print("Fraction:", float(np.nansum(contour_active_w.astype(int))) / float(winter_np.sum()))
    ...
    ds_out = xr.Dataset(
        {
            ...
            # kept: DP-tracked convective center (used by Step4)
            ...
            # new diagnostics: -15 contour west/east/centroid tracks
            "olr_thr_west_lon": xr.DataArray(
                west_lon.astype(np.float32),
                coords={"time": olr_bp_2d["time"]},
                dims=("time",),
                attrs={"desc": f"westernmost lon where olr_recon(latmean)<= {OLR_MIN_THRESH} (strongest segment)"}
            ),
            "olr_thr_east_lon": xr.DataArray(
                east_lon.astype(np.float32),
                coords={"time": olr_bp_2d["time"]},
                dims=("time",),
                attrs={"desc": f"easternmost lon where olr_recon(latmean)<= {OLR_MIN_THRESH} (strongest segment)"}
            ),
            "olr_thr_centroid_lon": xr.DataArray(
                contour_lon.astype(np.float32),
                coords={"time": olr_bp_2d["time"]},
                dims=("time",),
                attrs={"desc": f"centroid lon of strongest contiguous segment where olr_recon(latmean)<= {OLR_MIN_THRESH}"}
            ),
        },
        attrs={
            "step": "Step3 MV-EOF + reconstructed OLR + DP center track; event ID uses -15 OLR contour movement (paper-style)",
