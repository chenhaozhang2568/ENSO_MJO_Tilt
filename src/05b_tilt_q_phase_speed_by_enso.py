# -*- coding: utf-8 -*-
"""
05b_tilt_q_phase_speed_by_enso.py

按 ENSO 背景分类 MJO 事件，统计 tilt_q 和相速度，
进行组间 t 检验和 Mann-Whitney U 检验。

ENSO 分类：基于 NOAA ONI（与 05_tilt_by_enso.py 一致）
  - El Nino:  ONI >= +0.5
  - La Nina:  ONI <= -0.5
  - Neutral:  -0.5 < ONI < +0.5
"""
...
FIG_DIR      = Path(r"E:\Projects\ENSO_MJO_Tilt\outputs\figures\enso_tilt_q")
OUT_CSV      = r"E:\Datas\Derived\tilt_q_phase_speed_by_enso.csv"
...
def print_group_stats(df, var, label):
    """Print statistics for each ENSO group"""
    print(f"\n{'='*55}")
    print(f"  {label}")
    print(f"{'='*55}")
...
def print_significance(df, var, label):
    """Group comparisons: t-test and Mann-Whitney U"""
...
def _add_significance_brackets(ax, df, var, enso_order):
    """在箱线图上方添加所有组间 p 值括号"""
    pairs = [(0, 1), (1, 2), (0, 2)]  # positions: El Nino=1, Neutral=2, La Nina=3
    pair_labels = [
    ...
    y_range = ymax - ymin
    h_step = y_range * 0.06   # 每层间距
    y_start = ymax + y_range * 0.02  # 第一层起始位置
...
def plot_box_and_bar(df, out_dir):
    """绘制 tilt_q 和 phase_speed 的箱线图（含 p 值标注）"""
    fig, axes = plt.subplots(1, 2, figsize=(12, 6))

    for col_idx, (var, ylabel, title) in enumerate([
        ("mean_tilt_q", "Tilt_q (deg)", "Tilt_q by ENSO Phase"),
    ...
    fig.suptitle("MJO Tilt_q and Phase Speed by ENSO Phase",
...
def main():
    ...
    print("Computing event-mean tilt_q...")
    df_tilt = compute_event_mean_tilt_q()
    ...
    # Statistics
    print_group_stats(df, "mean_tilt_q", "Tilt_q by ENSO")
    print_group_stats(df, "phase_speed_m_s", "Phase Speed by ENSO")

    # Significance
    print_significance(df, "mean_tilt_q", "Tilt_q")
